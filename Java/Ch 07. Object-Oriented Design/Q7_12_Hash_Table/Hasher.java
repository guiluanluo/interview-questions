...
public class Hasher<K, V> {
  ...
  public V put(K key, V value) {
    ...
    if (arr.get(index) != null) {
      node.next = arr.get(index); // add the node to the form of the linked list - LUCY
      node.next.prev = node;
    }
    arr.set(index, node);
    return null;
  ...
  private LinkedListNode<K, V> getNodeForKey(K key) {
    ...
    while (current != null) {
      if (current.key == key) {
        System.out.println("** getNodeForKey() found node for key:" + key + ", index:" + index);
        return current;
      }
  ...
  /**
   * kinked list node class. used only with hash table. no one else should get access to this.
