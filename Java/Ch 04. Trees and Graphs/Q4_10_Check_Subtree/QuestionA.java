...
/**
 * TODO: review it later, jan 20, 2018, LL
 *
 ...
 * duplicate vlaues but different structures. the pre-order traversal will look the same even though T2 is a not a
 * subtree of T1. how can you handle situation like this?
 *
 * hint 31: although the problem seems liks it stems from duplicate values, it's really deeper that that. the issue is
 * that the pre-order traversal is the same only because there are null nodes that we skipped over(because the are
 * null). consider inserting a placeholder value into the pre0order traversal string whenever you reach a null node.
 * register the null node as a "real" node so that you can distinguish between the different stuctures.
 *
