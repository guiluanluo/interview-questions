 ...
 * @see <a href="https://en.wikipedia.org/wiki/Binary_heap">Binary Heap (Wikipedia)</a> <br>
 */
  ...
  public static class BinaryHeapTree<T extends Comparable<T>> implements BinaryHeap<T> {
    ...
    private Node<T> getNode(Node<T> startingNode, T value) {
      ...
      if (startingNode != null && startingNode.value.equals(value)) {
        ...
        if (left != null
            && ((type == Type.MIN && left.value.compareTo(value) <= 0) || (type == Type.MAX
            && left.value.compareTo(value) >= 0))
            ) {
        ...
        if (right != null
            && ((type == Type.MIN && right.value.compareTo(value) <= 0) || (type == Type.MAX
            && right.value.compareTo(value) >= 0))
            ) {
    ...
    protected void heapUp(Node<T> nodeToHeapUp) {
      ...
      while (node != null) {
        ...
        if ((parent != null) &&
            ((type == Type.MIN && node.value.compareTo(parent.value) < 0) || (type == Type.MAX
    ...
    protected void heapDown(Node<T> nodeToHeapDown) {
      ...
      if ((left != null && right != null) &&
        ...
        // Both children are greater/lesser than node
        if ((type == Type.MIN && right.value.compareTo(left.value) < 0) || (type == Type.MAX
            && right.value.compareTo(left.value) > 0)) {
          // Right is greater/lesser than left
          nodeToMove = right;
        } else if ((type == Type.MIN && left.value.compareTo(right.value) < 0) || (type == Type.MAX
