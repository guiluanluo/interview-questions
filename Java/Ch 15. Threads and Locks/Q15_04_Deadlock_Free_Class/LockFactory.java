...
public class LockFactory {
  ...
  private LockNode[] locks; /* represent a graph */
  /*
   * Maps from a process or owner to the order that the owner claimed it would call the locks in
   * keyed by ownId, value is the list of required locks in order
   */
  ...
  public boolean declare(int ownerId, int[] resourcesInOrder) {
    ...
    //keyed by resourceId, values is touched flag
  ...
  public Lock getLock(int ownerId, int resourceID) {
    ...
    //after call the lock, we remove it from the list to make sure locks are called in order!!
