 ...
 *
 * Note: use a BST as a hash table entry to hold objects with the same hash code (collisions). However, due to a BST
 * having O(log2 n) lookup and insertion, that means the worst case for the whole structure (hash table plus BST) is
 * technically better than using a list as a table entry.
 */
