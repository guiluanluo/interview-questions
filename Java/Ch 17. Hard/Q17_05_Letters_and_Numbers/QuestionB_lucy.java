...
public class QuestionB_lucy {
  ...
  public static int[] findLongestMatch(int[] deltas) {
    ...
    HashMap<Integer, Integer> deltaIndexMap = new HashMap<>();
    int[] max = new int[2];

    for (int i = 0; i < deltas.length; i++) {
      if (!deltaIndexMap.containsKey(deltas[i])) {
        deltaIndexMap.put(deltas[i], i);
      } else {
        int matchIndex = deltaIndexMap.get(deltas[i]);
