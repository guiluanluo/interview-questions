  ...
  private static PartitionResult partition(int[] array, int start, int end, int pivot) {
    ...
    while (middle <= right) {
      if (array[middle] < pivot) {
        /* Middle is smaller than the pivot. Left is either smaller or equal to the pivot.
    ...
    System.out.println( "[left:" + left +", middle:"+ middle +", right:"+ right
        + "], leftSize:" + (left - start) +", middleSize:" + (right - left + 1)
        +", start:" + start +", end:" + end +", pivot:" + pivot +", array:" + AssortedMethods.arrayToString(array));
